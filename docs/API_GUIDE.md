...
- **Question variety.** Each question step of a lesson is swapped per student for a same-type, same-marks, similar-difficulty variant from the lesson's practice pool. On a re-attempt the student only gets variants they have never answered; when none are left the least recently seen ones are used and the lesson is queued for the AI generator, which writes new variants (validated, tagged `ai-variant`, published) in the background. The generator also runs after any lesson completed below the pass mark. Settings: `Questions.GenerateVariants`, `Questions.MinUnseenPerActivity`, `Questions.MaxGeneratedPerDay`. Admins can trigger it: `POST /admin/lessons/{id}/generate-variants?count=1..3`.
